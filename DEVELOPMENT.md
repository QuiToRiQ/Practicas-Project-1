...
The entities in `backend/src/core/storage.pg/entities/` are the source of truth for the schema. Tables are created and evolved by TypeORM migrations under `backend/src/core/storage.pg/migrations/`, applied automatically at boot (`migrationsRun: true`, `synchronize: false`). The shape:
...
`StorageModule.forRoot()` runs with `synchronize: false` and `migrationsRun: true`, so any pending migration in the `migrations: [...]` array is applied at boot. The standalone CLI `DataSource` lives at `backend/src/core/storage.pg/data-source.ts` and is what the npm scripts point at.

When an entity changes, generate the migration from inside the backend container so it sees the running DB:

```bash
docker compose exec backend npm run migration:generate -- src/core/storage.pg/migrations/<DescriptiveName>
```

Then **append the generated class to the `migrations: [...]` array** in `backend/src/core/storage/storage.module.ts`. The runtime uses an explicit array (the CLI globs, the runtime doesn't), so a forgotten entry means the migration never runs and the next deploy crashes with `relation/column does not exist`. Commit the migration file and the module change together.

Other useful scripts (defined in `backend/package.json`):

```bash
npm run migration:create  -- src/core/storage.pg/migrations/<Name>   # empty migration (data fixes, raw SQL)
npm run migration:run                                                # apply pending (also runs at boot)
npm run migration:revert                                             # roll back the last applied migration
```

Never re-enable `synchronize`. If you need to bootstrap a fresh DB (e.g., a new contributor), `migrationsRun: true` will create everything from scratch on first boot — no manual baselining required.
